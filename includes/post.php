...
defined('CACHEINDEX') or define('CACHEINDEX', CACHEPATH . 'index.json');//This file name is to be enoceded with md5
...
class Post {
	...
	function __construct(){
		...
		$this->basefields = array('id', 'slug', 'title', 'status');
		...
		if (empty($data)){
			die('<br>Couldn\'t obtain Google spreadsheet with Key="'.GSSKEY.'" and SheetID="' . GSSSHEETID .'". Please verify and correct config file accordingly.');
		}
	...
	function readListedPosts($data, $fields){
		...
		foreach ($data->table->rows as $cols){
			...
			foreach ($data->table->cols as $c => $rh){
				//date must com from f instead of v;
				if ($rh->{'type'} == 'date') {
	...
	function getTagArchives($tagname, $page=1, $limit=20, $orderby=null){
		...
		$template = 'select %s where lower(' . $this->convertSlug2Id('tags') .') matches \'(^|.*,)\s*' . strtolower($tagname) . '\s*(,.*|$)\' and ' . $this->default_filter;
	...
	function searchPostsByContent($searchterm, $page=1, $limit=20, $orderby=null){
		...
		$template = 'select %s where lower(' . $this->convertSlug2Id('content') .') contains \'' . strtolower($searchterm) . '\' and ' . $this->default_filter;
		$orderby = $this->getOrderBy($orderby);
		$allfields = $this->header['slug'];
		return $this->readPosts($template, $allfields, $page, $limit, $orderby);
	}
	...
	function readAllTags(){
		...
		$append = 'select ' . $this->convertSlug2Id('tags') . ', count(' . $this->convertSlug2Id('id') . ') where ' . 
			...
			$this->default_filter .
			' group by ' . $this->convertSlug2Id('tags');
	...
	function findByUri($uri){
		...
		$this->post_content = do_shortcode( ob_get_contents() );
		ob_end_clean();
